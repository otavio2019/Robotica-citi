...
const validPayload = {
  ...
  team: { name: 'Equipe A', state: 'PB', city: 'Patos', level: 'LEVEL_4' as const, modalities: ['SUMO'], marketingCompetitorIndex: 0 },
  competitors: [
...
describe('regras do cadastro TJR', () => {
  ...
  it('aceita equipe de garagem sem INEP', () => expect(validateRegistration({ ...validPayload, institution: undefined, team: { ...validPayload.team, isGarage: true } }, 4).isGarage).toBe(true));
  ...
  it('exige modalidade e declaração', () => { expect(() => validateRegistration({ ...validPayload, team: { ...validPayload.team, modalities: [] } }, 4)).toThrow('modalidade'); expect(() => validateRegistration({ ...validPayload, acceptedDeclaration: false }, 4)).toThrow('aceitar'); });
